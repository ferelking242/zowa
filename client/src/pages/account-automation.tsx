...
import type { AutomationTask, Screenshot } from '@shared/schema';
...
export default function AccountAutomation() {
  ...
  const [expandedScreenshot, setExpandedScreenshot] = useState<string | Screenshot | null>(null);
  const logsEndRef = useRef<HTMLDivElement>(null);
  const debugLogsEndRef = useRef<HTMLDivElement>(null);

  const getScreenshotImage = (screenshot: string | Screenshot): string => {
    return typeof screenshot === 'string' ? screenshot : screenshot.image;
  };

  const getScreenshotDescription = (screenshot: string | Screenshot, index: number): string => {
    if (typeof screenshot === 'string') {
      return `Capture ${index + 1}`;
    }
    const parts = [`Tentative ${screenshot.attemptNumber}/${3}`];
    if (screenshot.description) {
      parts.push(`- Capture ${index + 1}`);
      parts.push(screenshot.description);
    } else {
      parts.push(`- Capture ${index + 1}`);
    }
    return parts.join('\n');
  ...
  const getStepIcon = (status: 'pending' | 'running' | 'completed' | 'failed') => {
    switch (status) {
      case 'pending':
        return <div className="rounded-full bg-gray-200 dark:bg-gray-700 h-8 w-8 flex items-center justify-center" title="En attente"><Clock className="h-4 w-4 text-gray-500" /></div>;
      case 'running':
        return <div className="rounded-full bg-blue-500 h-8 w-8 flex items-center justify-center" title="En cours"><Loader2 className="h-4 w-4 text-white animate-spin" /></div>;
      case 'completed':
        return <div className="rounded-full bg-green-500 h-8 w-8 flex items-center justify-center" title="Terminé"><CheckCircle className="h-4 w-4 text-white" /></div>;
      case 'failed':
        return <div className="rounded-full bg-red-500 h-8 w-8 flex items-center justify-center" title="Échoué"><XCircle className="h-4 w-4 text-white" /></div>;
    }
  };

  const getStepEmoji = (status: 'pending' | 'running' | 'completed' | 'failed') => {
    switch (status) {
      case 'pending':
        return '⏸️';
      case 'running':
                            ...
                            <div key={step.id} className="flex gap-3" data-testid={`step-${step.id}`}>
                              ...
                              <div className="flex-1 pb-3">
                                <div className="font-medium text-sm flex items-center gap-2">
                                  <span className="text-xl">{getStepEmoji(step.status)}</span>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {currentTask.screenshots.map((screenshot, index) => (
                            <div key={index} className="space-y-2">
                              <div className="flex items-center justify-between">
                                <div className="text-sm font-medium whitespace-pre-line" data-testid={`screenshot-label-${index}`}>
                                  📸 {getScreenshotDescription(screenshot, index)}
                                </div>
                                <Button
                                  variant="ghost"
                                  size="sm"
                              ...
                              <img
                                src={getScreenshotImage(screenshot)}
      ...
      {expandedScreenshot && (
        ...
        >
          <div className="max-w-6xl max-h-full">
            <img
              src={getScreenshotImage(expandedScreenshot)}
