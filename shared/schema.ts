...
export const automationTaskSchema = z.object({
  ...
  status: z.enum(['pending', 'running', 'completed', 'failed', 'paused', 'stopped']),
  steps: z.array(automationStepSchema).default([]),
  screenshots: z.array(z.union([z.string(), screenshotSchema])).default([]),
  logs: z.array(z.string()).default([]),
  debugLogs: z.array(z.string()).default([]),
  errorMessages: z.array(z.string()).default([]),
  createdAt: z.number(),
  completedAt: z.number().optional(),
  currentAttempt: z.number().default(1),
  isPaused: z.boolean().default(false),
  isStopped: z.boolean().default(false),
});
